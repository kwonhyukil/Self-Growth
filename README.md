...
# 🌱 Self Growth Log API<br>(自己成長ログ API)
...
- [아키텍처 및 흐름 (Architecture)](#-아키텍처-및-흐름-architecture)
- [주요 기능 (Key Features)](#-주요-기능-key-features)
- [데이터베이스 설계 (ERD)](#-데이터베이스-설계-erd)
- [API 명세 (API Specification)](#-api-명세-api-specification)
...
## 🏗 시스템 아키텍처 및 데이터 흐름 (Architecture)

본 프로젝트는 유지보수성과 테스트 용이성을 높이기 위해 **계층형 아키텍처(Layered Architecture)**를 채택했습니다.
비즈니스 로직은 서비스(Service) 계층에 집중시키고, 컨트롤러(Controller)는 클라이언트의 요청과 응답 처리에만 집중하도록 얇게(Thin) 설계했습니다.

### 🔄 API 요청 처리 흐름 (Request Flow)
...
### 1. 코칭형 일본어 피드백 엔진 (Feedback Engine)
...
## 📡 API 엔드포인트 명세 요약 (API Specification)
...
## 🗺 향후 확장 계획 (Roadmap / Future Scope)

현재 핵심 루프(검사 → 수정 → 통계)와 운영 최소 세트는 완성되었으며, 지속적인 고도화를 위해 아래와 같은 기능 확장을 계획하고 있습니다.

- [ ] **코칭 고도화 (Suggestion Apply):** 사용자가 피드백을 더 쉽게 반영할 수 있도록 템플릿 제공 및 빈칸 채우기 방식의 인터랙티브 코칭 도입.
- [ ] **개인화 리포트 (Analytics):** 누적된 `ja_revisions` 데이터를 바탕으로 주간 리포트 자동 생성 및 자주 틀리는 문법에 대한 '약점 히트맵(Heatmap)' 제공.
...
>
> 프로젝트에 대한 피드백이나 기여(Contribution)는 언제나 환영합니다!
