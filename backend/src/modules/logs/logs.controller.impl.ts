...
import { UpdateLogBody } from "./logs.schema";
...
/** POST /logs/:id/draft-ja — 한국어 칭찬 → AI 일본어 초안 생성 */
export async function draftJa(
  req: AuthRequest,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const result = await logsService.generateDraftJa(userId, logId);
    return ok(res, result, 200);
  } catch (err) {
    next(err);
  }
