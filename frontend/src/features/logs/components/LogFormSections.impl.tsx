...
import { JA_MAX_LEN, JA_MIN_LEN, MOOD_EMOJI, MOOD_FEEDBACK, MOOD_LABELS } from '@/shared/lib/constants'
...
export function LogFormStepIndicator({ current }: { current: number }) {
  return (
...
export function LogFormStepMood({
  happenedAt,
  moodTag,
  moodIntensity,
  onHappenedAtChange,
  ...
  onMoodTagChange: (value: MoodTag) => void
  onMoodIntensityChange: (value: number) => void
}) {
  return (
    <>
      ...
      <Input
        ...
        type="datetime-local"
        value={happenedAt}
        onChange={(e) => onHappenedAtChange(e.target.value)}
        required
      />
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          step={1}
          value={moodIntensity}
          onChange={(e) => onMoodIntensityChange(Number(e.target.value))}
...
export function LogFormStepEvent({
  triggerKo,
  specificEvent,
  onTriggerKoChange,
  onSpecificEventChange,
}: {
  triggerKo: string
  specificEvent: string
  onTriggerKoChange: (value: string) => void
  onSpecificEventChange: (value: string) => void
}) {
  return (
    <>
      <p className="mb-4 -mt-2 text-center text-sm text-text-soft">
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          value={triggerKo}
          onChange={(e) => onTriggerKoChange(e.target.value)}
          rows={3}
          maxChars={200}
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          value={specificEvent}
          onChange={(e) => onSpecificEventChange(e.target.value)}
          rows={3}
          maxChars={500}
...
export function LogFormStepReflection({
  praiseKo,
  praiseJa,
  savedLogId,
  drafting,
  ...
  onPraiseKoChange: (value: string) => void
  onPraiseJaChange: (value: string) => void
  onDraftJa: () => void
}) {
    ...
    <>
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          value={praiseKo}
          onChange={(e) => onPraiseKoChange(e.target.value)}
          rows={3}
          required
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <Textarea
            ...
            value={praiseJa}
            onChange={(e) => onPraiseJaChange(e.target.value)}
            rows={3}
            error={
              jaInvalid ? `${JA_MIN_LEN}〜${JA_MAX_LEN}文字にしてください（現在 ${jaLen} 文字）` : undefined
            }
          />
          {jaLen > 0 && (
...
export function LogFormStepReview({
  happenedAt,
  moodTag,
  moodIntensity,
  triggerKo,
  ...
  return (
    <div className="sticky bottom-0 -mx-6 -mb-6 mt-6 flex items-center justify-between gap-3 border-t border-border-subtle bg-surface-elevated px-6 py-4">
      ...
      <div className="ml-auto">
        {step < 4 ? (
          <Button type="button" onClick={onNext} disabled={!stepValid}>
            次へ
          </Button>
        ) : (
