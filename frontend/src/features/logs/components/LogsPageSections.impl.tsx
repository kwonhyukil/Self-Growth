      ...
      <div className="grid gap-8 px-7 py-8 md:grid-cols-[1.2fr_0.8fr] md:px-10 md:py-10">
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Button onClick={onCreate} leftIcon={<span>✍</span>} size="lg">
              새 로그 쓰기
            </Button>
            {logCount > 0 && (
              <Badge variant="outline" className="rounded-full px-4 py-2 text-xs">
                Saved journals {logCount}
              </Badge>
            )}
          </div>
        </div>
...
export function LogsPageArchive({ logs }: { logs: GrowthLog[] }) {
  return (
...
export function LogsPageEmptyState({ onCreate }: { onCreate: () => void }) {
  return (
