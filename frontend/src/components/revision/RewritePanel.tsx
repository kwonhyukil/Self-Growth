...
import { JA_MIN_LEN, JA_MAX_LEN } from '../../utils/constants'
import { useRewriteJa, useJaRevisions } from '../../hooks/useJaCheck'
import { Button } from '../ui/Button'
import { Textarea } from '../ui/Input'
import { ErrorMessage } from '../ui/ErrorMessage'
import { ScoreBadge } from '../jaCheck/ScoreBadge'
import { deltaLabel } from '../../utils/formatters'
...
export function RewritePanel({ log, latestFeedback, onClose }: RewritePanelProps) {
  ...
  return (
    <div className="rounded-2xl border-2 border-brand-200 bg-brand-50 p-5 space-y-5 animate-slide-up">
      ...
      {/* Original */}
      <div className="rounded-lg bg-white/70 px-4 py-3 border border-brand-100">
        <p className="text-xs text-slate-400 mb-1">元のテキスト</p>
        <p className="text-sm text-slate-700">{log.praiseJa}</p>
      </div>

      {/* Issues to address */}
      {latestFeedback && latestFeedback.issues.length > 0 && (
        <div className="space-y-1">
          <p className="text-xs font-semibold text-brand-700">修正すべき点:</p>
          {latestFeedback.issues.slice(0, 3).map((issue) => (
            <div key={issue.issueId} className="flex items-start gap-2 text-xs text-brand-800">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Result */}
        {rewrite.data && (
          <div className="rounded-xl bg-white border border-emerald-200 p-4 space-y-2 animate-slide-up">
            <p className="text-sm font-semibold text-slate-700">書き直し結果</p>
            <div className="flex items-center gap-4">
              <div className="text-center">
                <p className="text-xs text-slate-400 mb-1">前</p>
              ...
              <div className="flex-1 text-center">
                <p className={clsx(
                  'text-sm font-bold',
                  (rewrite.data.delta.issueCount ?? 0) < 0 ? 'text-emerald-600' : 'text-slate-500'
                )}>
                  {deltaLabel(rewrite.data.delta.issueCount)}
                </p>
              </div>
              <div className="text-center">
                <p className="text-xs text-slate-400 mb-1">後</p>
